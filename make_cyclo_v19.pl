...
GetOptions ("schedule|s=s" => \my $sogl, "debug|d"=> \my $debug, "usage|u"=>\my $usage, "interactive|i"=>\my $interactive); 
if($usage){
	print "USAGE : perl $0 --s single_schedule_file(after break_sns.pl) [--debug]\n";
	exit 0;
}
...
(my $srt_options,my $srt_errors)=read_soglasnov(\@g,\%S);	# schedule reading routine
...
foreach (sort keys %S){
	if($S{$_}{'type'} eq "sns_cmd"){
		if($next){print print_time($S{$_}{'start'}),"\n" if $debug2; $global_stop_sec=$S{$_}{'start'}; }
		else{print print_time($S{$_}{'stop'}),"\n" if $debug2;$next=1;$global_start_sec=$S{$_}{'stop'};}
	}
}
...
my $lastobskey;
foreach (sort keys %S){
	if($S{$_}{'type'} eq "obs" || $S{$_}{'type'} eq "just" || $S{$_}{'type'} eq "just_virk"  ){
...
my $firstvirkkey;	# key of the first observation/justirovka_virk in cyclogramm. That uses POWER 
my $lastvirkkey;
foreach (sort keys %S){
...
my $time_of_prev_stop = $S{$firstobskey}{'stop'};	# stop time of previous observation
my %receiver_poweron=();				# hash with keys like ones in the %S hash. If key exists here then BEFORE this observation receivers should be powered on
my %receiver_poweroff=();				# hash with keys like ones in the %S hash. If key exists here then AFTER(not checked yet) this observation receivers should be powered off
...
foreach (sort keys %S){
	...
	if($S{$_}{'type'} ne "obs" && $S{$_}{'type'} ne "just" && $S{$_}{'type'} ne "just_virk"  ){
	    next;
	}
...
my @rec_periods = ($firstobskey, sort keys %receiver_poweroff, $global_stop_sec);
...
for (my $i =1; $i<scalar @rec_periods; $i++){
    ...
    foreach (sort keys %S){
	if($S{$_}{'start'} < $rec_periods[$i-1] or $S{$_}{'stop'} > $rec_periods[$i]){next;}	# skip observations not from this period.
	if($S{$_}{'type'} ne "obs" && $S{$_}{'type'} ne "just" && $S{$_}{'type'} ne "just_virk" ){next;}
# 	print ".\n";
...
# 	print "bands num = ",$S{$_}{'bands_num'},"\n";
	...
	for my $i (0..$S{$_}{'bands_num'}){
		push @bands_inthisperiod,substr($S{$_}{'bands'.$i},0,1)."1";
		push @bands_inthisperiod,substr($S{$_}{'bands'.$i},1,1)."2";
		
		if($S{$_}{'type'} eq 'just' ){ # 4 channels totally available for justirovka
			push @bands_inthisperiod,substr($S{$_}{'bands'.$i},0,1)."2" if substr($S{$_}{'bands'.$i},0,1)!~m/c/i;
			push @bands_inthisperiod,substr($S{$_}{'bands'.$i},1,1)."1" if substr($S{$_}{'bands'.$i},1,1)!~m/c/i;
		}
	}
...
# 	print Dumper(@bands_inthisperiod);
    ...
    print print_time($rec_periods[$i-1]) ," -- ", print_time($rec_periods[$i]),"\t BANDS = ", @bands_inthisperiod,"\n";
...
foreach(sort keys %receiver_poweron){
    print "in advance before ",print_time($_), " power on ",join(" ",@{$receiver_poweron{$_}}),"\n";
}

print "POWER OFF rec:\n";
foreach(sort keys %receiver_poweroff){
    print "after ",print_time($_), " power off ",join(" ",@{$receiver_poweroff{$_}}),"\n";
}
...
foreach (sort keys %S){
	if($S{$_}{'type'} eq "obs" || $S{$_}{'type'} eq "just" || $S{$_}{'type'} eq "just_virk"  ){
		...
		for my $i (0..$S{$_}{'bands_num'}){
			...
			if($S{$_}{'type'} eq 'just' ){ # 4 channels totally available for justirovka
				push @allbands,substr($S{$_}{'bands'.$i},0,1)."2" if substr($S{$_}{'bands'.$i},0,1)!~m/c/i;
				push @allbands,substr($S{$_}{'bands'.$i},1,1)."1";
			}
		}
	}
}
@start_times=sort {$a <=> $b} @start_times;		# this 2 shouldn't be reasonable if everything is ok
@stop_times=sort {$a <=> $b} @stop_times;		# this 2 shouldn't be reasonable if everything is ok
...
if($use_k1==1 and $use_k2==1){
	for my $i (0..$#uniq_to_poweron){
		if($uniq_to_poweron[$i]=~m/k1/i){splice @uniq_to_poweron,$i,1; last;}
	}
}
...
my @keys=();
foreach(sort keys %S){
	if($S{$_}{'type'} eq "obs" || $S{$_}{'type'} eq "just_virk" || $S{$_}{'type'} eq "just"){
		push @keys,$_;
	}
}
my @cmd=();
...
# 	print $rec,"\t", $S{$keys[$i]}{'bands'},"\n";;
	...
	if($S{$keys[$i]}{'type'} eq 'just'){	# power on both channels
	    if(lc(substr($S{$keys[$i]}{'bands'},0,1)) eq lc(substr($rec,0,1))  or lc(substr($S{$keys[$i]}{'bands'},1,1)) eq lc(substr($rec,0,1))){
		    push @array, $keys[$i];
	    }
	}
	else{
	    if($rec =~ m/1/ and lc(substr($S{$keys[$i]}{'bands'},0,1)) eq lc(substr($rec,0,1))){
		push @array, $keys[$i];
	    }
	    if($rec =~ m/2/ and lc(substr($S{$keys[$i]}{'bands'},1,1)) eq lc(substr($rec,0,1))){
		push @array, $keys[$i];
	    }
	}
    }
...
my %all_rec_poweron;	# keys of observation BEFORE which rec should be powered on. Primary key - rec. Value - array of %S keys( == start times)
...
foreach(keys %all_rec_periods){    
    ...
    if(scalar @{$all_rec_periods{$_}} == 0){die "Zero time usage of receiver. Weirdest thing. Bye.\n";}
    elsif(scalar @{$all_rec_periods{$_}} == 1){		# if only 1 observation: power on before, power off after. Simple.
...
# 	print $_,"\t";
	for(my $i=1; $i<scalar @{$all_rec_periods{$_}};$i++){
	    ...
	    if(($S{${$all_rec_periods{$_}}[$i]}{'start'} - $S{${$all_rec_periods{$_}}[$i-1]}{'stop'}) >= 5*3600 ){	# if a gap between start of this obs and stop of previos with this rec is longer than 5hours
		# if found than $i-th is pushed to poweron, $i-1 th  -- to poweroffs
	    
		print "gap between ",$S{${$all_rec_periods{$_}}[$i]}{'obscode'}, "  and  ", $S{${$all_rec_periods{$_}}[$i-1]}{'obscode'}, " is ", ($S{${$all_rec_periods{$_}}[$i]}{'start'} - $S{${$all_rec_periods{$_}}[$i-1]}{'stop'})/3600 , " hours\n";
...
foreach (sort keys %S){
	...
	if($keys[$i] == $this){
	  for my $back(1..5){
	    if($S{$keys[$i-$back]}{'type'} eq "obs" or $S{$keys[$i-$back]}{'type'} eq "just_virk"){
	  ...
	  for my $forward(1..5){
	    if($S{$keys[$i+$forward]}{'type'} eq "obs" or $S{$keys[$i-$forward]}{'type'} eq "just_virk"){
...
$S{$_}{'bands'}=~s/[0-9\-\+]//g;
	...
	if($S{$_}{'type'} eq "obs" || $S{$_}{'type'} eq "just_virk"){
		...
		if($S{$_}{'type'} eq "just_virk" and $S{$_}{'var'} == 0){
			...
			if ($S{$_}{'power'} == 40 and ( $_+0==$firstvirkkey+0 or (0+$this-$prevstop)>900 )){
					#$t-=$dt;
					$t=$S{$_}{'beginscan'};
					
					print "POWER = 40 ON at ",print_time($t),"\n";
					my @cmd="1\t".$dt."\t3112\t// otkl. regim 4W (power = 40W)";
					
					
					# added deep in the night 04.07.2014
					print "FGSVCH on (different bands)\n";
					my @cmd1=read_file(uc(substr($S{$_}{'bands'},0,1))."1/".lc(substr($S{$_}{'bands'},0,1))."1_fgsvch_on");					
					my @cmd2=read_file(uc(substr($S{$_}{'bands'},1,1))."2/".lc(substr($S{$_}{'bands'},1,1))."2_fgsvch_on");					
					#unshift @cmd,@cmd2;
					#unshift @cmd,@cmd1;
					push @cmd,@cmd1;
					...
					if(substr($S{$_}{'bands'},0,1) =~ m/k/i){
						push @cmd, "1\t".$dt."\t3230,2443A000\t// 2F0 7/8dB GSH-otkl U 7.5mA";
					}
										
					push @cmd,@cmd2;
					if(substr($S{$_}{'bands'},1,1) =~ m/k/i and substr($S{$_}{'bands'},0,1) !~ m/k/i ){
						push @cmd, "1\t".$dt."\t3230,2443A000\t// 2F0 7/8dB GSH-otkl U 7.5mA";
					}
					insert_block(\$t,\@cmd,"-",1);
			}

			 my @cmd = read_file("GSH_NEW/".lc(substr($S{$_}{'bands'},0,1)).lc(substr($S{$_}{'bands'},1,1))."_var0");
			...
			if ($S{$_}{'power'} == 40 and (($next-$thisstop)>900 or  $S{$next}{'power'}==4) ){
			
				$t=$S{$_}{'stop'}+10;
# 				$t=$S{$_}{'endscan'};
				print "Power 40 OFF at ",print_time($t),"\n";
				my @cmd="1\t".$dt."\t3111\t// vkl. regim 4W (power =  4W)";
				insert_block(\$t,\@cmd,"+",1);
			}
		...
		##############################################################
		# GSH
		print "Calibration before ",$S{$_}{'obscode'}," start time ",print_time($S{$_}{'start'}),"\n";
		# GSH BEFORE
		my @cmd=();
		my @rep_cmd=();
		my @cmd1=();
		...
		if(
		(abs($S{$this}{'ts_bef'}+0) < 2 and
		($this-$prevstop)<600)){$doshort_bef=1;}
		
		if(($S{$this}{'ts_aft'}+0 < 2 and
		($next-$thisstop)<600)){
		  $doshort_aft=1;
		}
...
# 		unless($S{$_}{'obscode'} =~ m/(?:puts|gbts)/i   and  $S{$_}{'ts_mode'} !~ m/ch/i){
		...
		unless($doshort_bef){
			...
			if($S{$_}{'bands'}=~m/k/i){
				push @cmd,"1\t".$dt."\t3240,0000009E\t// otkl.kanalov FGSVCH";
				push @cmd,"1\t".(70-$dt)."\t3230,3F000000\t// otkl. get. 1.35";
			}		
			else{
			...
			if(substr($S{$_}{'bands'},0,1) eq substr($S{$_}{'bands'},1,1)){
...
# v19					push @cmd, "1\t".$dt."\t3230,2443A000\t// 2F0 7/8dB GSH-otkl U 7.5mA";
		    ...
		    if($S{$_}{'ts_bef'}<=0 && block_duration(\@rep_cmd) < (abs($S{$_}{'ts_bef'})-3)*60){	# if fits well
		    
		    
print "DEBUG: ts_bef=", $S{$_}{'ts_bef'}, " GSH before block duration = ",block_duration(\@rep_cmd),"\n"  if $debug;
			    ...
			    if($interactive){
				    ...
				    print "Do you want to perform calibration before this obervation[y/n]";
				    $ans=<STDIN>;
			    }
			    else{
				    $ans= "y";
			    ...
			    if($ans=~m/n/i){$dozu=0;$dogsh=0;}
				    ...
				    if($interactive){
					    print "Duration of calibration is ",&block_duration(\@rep_cmd)," sec\n";
					    print "Enter calibration time offset in SECONDS (with + or -):\n";
					    $ans=int(<STDIN>);
				    }
					    ...
					    if($S{$_}{'ts_bef'}+0 == 0){
						    ...
						    elsif($S{$_}{'bands'}=~m/([^k])[^\1k]/i){ # I believe this should correspond to lc,cl,lp,pl,cp,pc
							    $ans += 110;
						    }
		    ...
		    if($doshort_bef){
		      $t=$S{$_}{'start'}-15;
		      $GSHB{$S{$_}{'start'}}=$t;
		      @cmd = read_file("GSH_NEW/".lc($S{$_}{'bands'})."_gsh_short");
		      @rep_cmd=repeat_block(\@cmd,2);
		    }
		}
		...
		dogdh = $dogsh
		";
		print $S{$_}{'start'} - &block_duration(\@rep_cmd)," <= ", $S{$_}{'start'}+$S{$_}{'ts_bef'}*60-10 ,"\n";
		
		
		
		# if		max possible time of GSH_before start   <   time of power 4/40 switching 
		if ($dogsh and ($S{$_}{'start'} - &block_duration(\@rep_cmd))  <= ($S{$_}{'start'}+$S{$_}{'ts_bef'}*60-10) ){
				...
				if ($S{$_}{'power'}+0 == 40 ){
					...
					if($S{$prev}{'power'}+0==4 or ($this-$prevstop)>900  or $this==$firstvirkkey){
					...
					if($dogsh){
						insert_block(\$t,\@rep_cmd,"-",1) if $S{$_}{'ts_bef'}+0!=0;
						insert_block(\$t,\@rep_cmd,"+",1) if $S{$_}{'ts_bef'}+0==0;
					}
		...
		elsif(!$dogsh and ($S{$_}{'start'} - &block_duration(\@rep_cmd))  <= ($S{$_}{'start'}+$S{$_}{'ts_bef'}*60-10) ){
		    if ($S{$_}{'power'}+0 == 40 and  (($this-$prevstop)>900  or $this==$firstvirkkey)){
			...
			if ($S{$_}{'power'}+0 == 40 and  (($this-$prevstop)>900  or $this==$firstvirkkey)){
				$t=$S{$_}{'start'}+$S{$_}{'ts_bef'}*60-10;
				if($S{$prev}{'power'}+0==4 or ($this-$prevstop)>900 ){
...
# 		print Dumper(@rep_cmd);# if $debug;
		...
		# POWER 40 OFF
		 ...
		 print "this = $this   ",print_time($this),"\n";
			print "prev = $prevstop   ",print_time($prevstop),"\n";   
		print "\n++++++++++++++++\n",$this-$prevstop," seconds between observations\n";
		...
		if($S{$_}{'ts_mode'} =~ m/ch/i){
		  ...
		  $t=$S{$_}{'stop'}+&block_duration(\@rep_cmd);
		    ...
		    print "Calibration after ",$S{$_}{'obscode'}," stop time ",print_time($S{$_}{'stop'}),"\n";
			...
			if($S{$_}{'bands'}=~m/k/i){
				push @cmd,"1\t".$dt."\t3240,0000009E\t// otkl.kanalov FGSVCH";
				push @cmd,"1\t".$dt."\t3230,3F000000\t// otkl. get. 1.35";
			}		
			else{
				push @cmd,"1\t".$dt."\t3240,0000009E\t// otkl.kanalov FGSVCH";
			}
		    ...
		    if(&block_duration(\@rep_cmd) <= (abs($S{$_}{'ts_aft'}))*60){	# if fits well
			...
			print "DEBUG: ts_aft = ", $S{$_}{'ts_aft'}*60 , "\n ts aft duration =  ", &block_duration(\@rep_cmd) ,"\n" if $debug;
			    ...
			    if($interactive){
				    print "block duration is ",&block_duration(\@rep_cmd),"\n";
				    print "Do you want to perform calibration AFTER this obervation[y/n]";
				    $ans=<STDIN>;
			    }
			    else{
				    $ans="y";
...
# 		      $t=$S{$_}{'stop'} - 20;
		...
		unless($interactive){    
		  if ($S{$_}{'power'} == 40 and 	((($next-$thisstop)>900 ) or (($next-$thisstop)<900 and $S{$next}{'power'}==4) or ($this==$lastvirkkey))){
	...
	if($S{$_}{'type'} eq "just" ){
		...
		print "Calibration before JUSTIROVKA start time ",print_time($S{$_}{'start'})," beginscan ",print_time($S{$_}{'beginscan'}),"\n";
		my @ts=();
		my @gsh_pattern;
		my $ans="";
		my @cmd=();
		...
		if($S{$_}{'var'} == 0 || $S{$_}{'var'} == 1 || $S{$_}{'var'} == 2 || $S{$_}{'var'} == 3 || $S{$_}{'var'} == 4 ){
			$t=$S{$_}{'beginscan'};
			push @ts,$t;
		

			if(substr($S{$_}{'bands'},0,1) eq substr($S{$_}{'bands'},1,1)){	# same bands
			

				my @cmd1=read_file("GSH_NEW/".lc($S{$_}{'bands'})."_gsh_yust");
				push @cmd,@cmd1;
				@cmd1= read_file(uc(substr($S{$_}{'bands'},0,1))."1/".lc(substr($S{$_}{'bands'},0,1))."1_fgsvch_on");
				# v18
				# add command to the eng of the "fgsvch on" block
				if(substr($S{$_}{'bands'},0,1) =~ m/k/i){
					push @cmd1, "1\t".$dt."\t3230,2443A000\t// 2F0 7/8dB GSH-otkl U 7.5mA";
				}
				unshift @cmd,@cmd1;
				if($S{$_}{'bands'}=~m/k/i){
					unshift @cmd,"1\t".(70-$dt)."\t3230,3F000000\t// otkl. get. 1.35";
					unshift @cmd,"1\t".$dt."\t3240,0000009E\t// otkl.kanalov FGSVCH";
				}		
				else{
					unshift @cmd,"1\t".$dt."\t3240,0000009E\t// otkl.kanalov FGSVCH";
				}
...
# 			    
				...
				print "use GSH file : GSH_NEW/".lc($S{$_}{'bands'})."_gsh_yust\n" if $debug;
				...
				my @cmd2=read_file(uc(substr($S{$_}{'bands'},1,1))."2/".lc(substr($S{$_}{'bands'},1,1))."2_fgsvch_on");			# v18
				# add command to the eng of the "fgsvch on" block
				if(substr($S{$_}{'bands'},1,1) =~ m/k/i and substr($S{$_}{'bands'},0,1) !~ m/k/i){
					push @cmd2, "1\t".$dt."\t3230,2443A000\t// 2F0 7/8dB GSH-otkl U 7.5mA";
				}
...
# 			unshift @rep_cmd,"1\t".$dt."\t866-34\t// vkl zapis ZU";
			...
			do{
				...
				print "Enter space separated GSH pattern in seconds: offset(time of gsh on), gsh on duration, gsh off duration, number of repetitions:\n";
				my $t1=<STDIN>;
				chomp($t1);
				$t1=~s/\s+$//;
				$t1=~s/^\s+//;
...
#			print "Number of repetitions = ",$g{'num'},"\n" if $debug;
			...
			for my $i(1..$g{'num'}){
				push @ts,$t;
				print "$i: new GSH special time calculated: $t\t",print_time($t),"\n" if $debug;
				$t+=$g{'on'}+$g{'off'};
			}


			if(substr($S{$_}{'bands'},0,1) eq substr($S{$_}{'bands'},1,1)){	# same bands
				print "Trying to open file ","GSH_NEW/".lc($S{$_}{'bands'})."_gsh5\n" if $debug;
	
				my @cmd1=read_file("GSH_NEW/".lc($S{$_}{'bands'})."_gsh5");
				push @cmd,@cmd1;
			}
			else{		# different bands
				my @cmd1=read_file("GSH_NEW/".lc($S{$_}{'bands'})."_gsh5");
				my @cmd2=read_file("GSH_NEW/".reverse(lc($S{$_}{'bands'}))."_gsh5");
				push @cmd,@cmd1;
				push @cmd,@cmd2;
			}
		...
		print "Calibration AFTER JUSTIROVKA start time ",print_time($S{$_}{'stop'})," end ",print_time($S{$_}{'endscan'}),"\n";
		my @ts=();
		my @gsh_pattern;
		my $ans="";
		my @cmd=();
...
# 				
				...
				my @cmd1 = read_file("GSH_NEW/".lc($S{$_}{'bands'})."_gsh_yust_aft");
				push @cmd,@cmd1;
				if($S{$_}{'bands'}=~m/k/i){
					push @cmd,"1\t".$dt."\t3240,0000009E\t// otkl.kanalov FGSVCH";
					push @cmd,"1\t".$dt."\t3230,3F000000\t// otkl. get. 1.35";
...
for my $i (0..$#keys){
	...
	if($S{$st}{'type'} eq "obs" || $S{$st}{'type'} eq "just_virk"){
		...
		print "Kluchi on for ",$S{$st}{'obscode'}," starting at ",print_time($S{$st}{'start'}),"\n";
		#print " I = ",$i, "\n";
		if($i==0){
			$do1=1;
			$do2=1;			
			...
			my @cmd1=read_file(uc($b1)."1/".lc($b1)."1_kluchi_on",'all') if $do1; 		# if needed
			my @cmd2=read_file(uc($b2)."2/".lc($b2)."2_kluchi_on",'all') if $do2;
			push @cmd,@cmd1 if $do1;
			push @cmd,@cmd2 if $do2;
			push @cmd, "1\t5\t3240,000000AE\t// Otkl. shiny (1-8) +27V SSVCh" if ($do1 or $do2);
		...
		if($i > 0 and $i <= $#keys){
			...
			if($b1 eq substr($S{$keys[$i-1]}{'bands'},0,1)){	# if no switching needed for rec. 1
				print "don't need to switch kluchi for receiver 1\n";
				$do1=0;
			}

			if($b2 eq substr($S{$keys[$i-1]}{'bands'},1,1)){	# if no switching needed for rec. 2
				print "don't need to switch kluchi for receiver 2\n";
				$do2=0;
			}

			# first switch off both
			if($b1 ne substr($S{$keys[$i-1]}{'bands'},0,1)){	# neeed to switch off keys for b1
				$do1=1;
				# switch off
			...
			if($b2 ne substr($S{$keys[$i-1]}{'bands'},1,1)){	# neeed to switch off keys for b2
				$do2=1;
				# switch off
				print "FILE=",uc(substr($S{$keys[$i-1]}{'bands'},1,1))."2/".lc(substr($S{$keys[$i-1]}{'bands'},1,1))."2_kluchi_off","\n" if $debug2;
				my @cmd1=read_file(uc(substr($S{$keys[$i-1]}{'bands'},1,1))."2/".lc(substr($S{$keys[$i-1]}{'bands'},1,1))."2_kluchi_off",'all');
				push @cmd,@cmd1;
			}
			...
			if($b1 ne substr($S{$keys[$i-1]}{'bands'},0,1)){	# neeed to switch ON keys for b1
				# switch on
				print "FILE=",uc($b1)."1/".lc($b1)."1_kluchi_on","\n" if $debug2;
				my @cmd1=read_file(uc($b1)."1/".lc($b1)."1_kluchi_on",'all');
				push @cmd,@cmd1;
			}

		
			if($b2 ne substr($S{$keys[$i-1]}{'bands'},1,1)){	# neeed to switch ON keys for b2
				# switch on
				print "FILE=",uc($b2)."2/".lc($b2)."2_kluchi_on","\n" if $debug2;
				my @cmd1=read_file(uc($b2)."2/".lc($b2)."2_kluchi_on",'all');
				push @cmd,@cmd1;
			}
		...
		# replaces $i-1 with $i in indices of array
			# switch off
			print "FILE=",uc(substr($S{$keys[$i]}{'bands'},0,1))."1/".lc(substr($S{$keys[$i]}{'bands'},0,1))."1_kluchi_off","\n" if $debug2;
			my @cmd1=read_file(uc(substr($S{$keys[$i]}{'bands'},0,1))."1/".lc(substr($S{$keys[$i]}{'bands'},0,1))."1_kluchi_off",'all');
			push @cmd,@cmd1;



			# switch off
			print "FILE=",uc(substr($S{$keys[$i]}{'bands'},1,1))."2/".lc(substr($S{$keys[$i]}{'bands'},1,1))."2_kluchi_off","\n" if $debug2;
			my @cmd1=read_file(uc(substr($S{$keys[$i]}{'bands'},1,1))."2/".lc(substr($S{$keys[$i]}{'bands'},1,1))."2_kluchi_off",'all');
			push @cmd,@cmd1;
...
for my $i (1..$#keys){
	...
	if(lc($S{$st}{'fmode'}) eq "f3/f3" && lc($regim_f) ne "f3/f3" ){			# we need f3/f3, but f2/f2 is switched on
...
#		$t=$S{$st}{'start'}+$S{$st}{'ts_bef'}*60;
		...
		@cmd=();
		print "Switch to f3/f3\tCode: ",$S{$st}{'obscode'}," Start: ",&print_time($t),"\n";
...
#		$t=$S{$st}{'start'}+$S{$st}{'ts_bef'}*60;
		...
		insert_block(\$t,\@cmd,"-",2);
		@cmd=();
		print "Switch to f2/f2\tCode: ",$S{$st}{'obscode'}," Start: ",&print_time($t),"\n";
...
my $kband_ON = 0;		# flag that any of the K-band receivers is powered on (either K1 or K2)
...
for(my $i=0;$i<scalar @keys;$i++){
    ...
    my $poweroff_kband=1;		# power off the K-band receiver flag. To be set 0 if K-band should not be powered off.
...
BEFOBSON:			for (my $j=0;$j<$i;$j++){
						...
						if($rec !~ m/k/i){
							...
							if(lc( substr($S{$keys[$j]}{'bands'},0,1) ) eq lc( substr($rec,0,1)) and ($S{$keys[$j]}{'stop'} - $S{$keys[$i]}{'start'}) < 5*3600){
								# do nothing
	# 							die "soft\n";
							...
							# If there WERE previous observations with the same receiver within 5 hours before the current observation (so they should not be powered off yet)
							if(lc( substr($S{$keys[$j]}{'bands'},1,1) ) eq lc( substr($rec,0,1)) and ($S{$keys[$j]}{'stop'} - $S{$keys[$i]}{'start'}) < 5*3600){
								# do nothing
	# 							die "light\n";
							}
							else{
							...
							# Any channel: K1, K2
							# If there WERE previous observations with the same receiver within 5 hours before the current observation (so they should not be powered off yet)
							if((lc( substr($S{$keys[$j]}{'bands'},0,1) ) eq lc( substr($rec,0,1))  or  lc( substr($S{$keys[$j]}{'bands'},1,1) ) eq lc( substr($rec,0,1)) )  and ($S{$keys[$j]}{'stop'} - $S{$keys[$i]}{'start'}) < 5*3600){
								# do nothing
...
AFTEROBSOFF:		for (my $j=$i+1;$j<scalar @keys;$j++){
					...
					# v19
						if(($S{$keys[$j]}{'start'} - $S{$keys[$i]}{'stop'}) >= 5*3600){
						...
						if($rec !~ m/k/i){
							...
							if((lc( substr($S{$keys[$j]}{'bands'},0,1) ) eq lc( substr($rec,0,1)) or lc( substr($S{$keys[$j]}{'bands'},1,1) ) eq lc( substr($rec,0,1))) and ($S{$keys[$j]}{'start'} - $S{$keys[$i]}{'stop'}) < 5*3600 ){
								# do nothing
							}
							else{
								push @poweroff_now, $rec;
	...
	# 						if($rec=~m/k/i){die "OFF hard\n";}
							...
							print "bands = ",lc(substr($S{$keys[$j]}{'bands'},0,1)),"\n";
							print "bands = ",lc(substr($S{$keys[$j]}{'bands'},1,1)),"\n";
							
							if((lc(substr($S{$keys[$j]}{'bands'},0,1) ) eq lc(substr($rec,0,1))  or  lc(substr($S{$keys[$j]}{'bands'},1,1) ) eq lc(substr($rec,0,1)) )  ){
								# do nothing
# 								die "will I??\n";
...
#     print "kband_ON = $kband_ON \n"; 
	...
	print "OBS = ", $S{$keys[$i]}{'obscode'}, " \t poweron  ", join(" ", @poweron_now) ,"\n" if $debug;
...
# 	    print "prev time = glbal start\n";
	    ...
	    $prev_time = (defined $GSHA{$S{$keys[$i-1]}{'stop'}}?$GSHA{$S{$keys[$i-1]}{'stop'}}:$S{$keys[$i-1]}{'stop'}+5*60);
	}

	# define warm up time
	my $time2warmup = 7200;
	if(any {  m/[kc]/i } @poweron_now){$time2warmup = 7200;}	
	...
	if($i==0){
		if($time_to_start > $time2warmup) {
			...
			$time2warmup = $keys[$i] - $global_start_sec - 5*60;	# give 5 min on initial commands
		}
	}
...
# 	print "scalar poweron_now = ", scalar @poweron_now,"\n";
		...
		if(scalar @poweron_now){
			...
			if((any {$_ =~ m/k1/i} @poweron_now ) or (any {$_ =~ m/k2/i} @poweron_now) ){
			...
			if($S{$keys[$i]}{'ts_mode'} =~ m/ch/i and (any {$_ =~ m/c/i} @poweron_now)){
				for my $i (0..$#$ref){
					if($$ref[$i] =~ m/3130.*TRST/i){splice @$ref,$i,1; last;}
				}
# 				print Dumper($ref) and die;
			...
			if($S{$keys[$i]}{'type'} =~ /just/i  ){
				...
				if($i==0){$t=$global_start_sec+5*60;
					insert_block(\$t,\@cmd,"+",2);		# move into future
				}
				else{$t=$S{$keys[$i-1]}{'start'};
					insert_block(\$t,\@cmd,"-",2);		# move into past
				}
				}
			}
...
#     print $S{$keys[$i]}{'obscode'}, " warmup time = ", $time2warmup,"\n";
    ...
    if(scalar @poweroff_now){
    	print "OBS = ", $S{$keys[$i]}{'obscode'}, " \t powerOFF  ", join(" ", @poweroff_now) ,"\n" if $debug;
	$t = (defined $GSHA{$S{$keys[$i]}{'stop'}}?$GSHA{$S{$keys[$i]}{'stop'}}:$S{$keys[$i]}{'stop'}+5*60);
	...
	foreach my $r (@super_uniq_now){
	    ...
	    if($S{$keys[$i]}{'ts_mode'} =~ m/ch/i){
			for my $j (0..$#cmd){
				if($cmd[$j] =~ m/3132.*TRST/i){splice @cmd,$j,1; last;}
			}
	    }
	    insert_block(\$t,\@cmd,"+",2);
	    $t+=30;	# 30 sec
...
print "to this moment min JUST time is $firstobskey =  ",&print_time($firstobskey),"\n";
...
#		insert_block(\$t,\@cmd,"-",2);
#		@cmd=();
		print "Switch to f3/f3\tCode: ",$S{$keys[0]}{'obscode'}," Start: ",&print_time($t),"\n";
...
#		insert_block(\$t,\@cmd,"-",2);
#		@cmd=();
		print "Switch to f2/f2\tCode: ",$S{$keys[0]}{'obscode'}," Start: ",&print_time($t),"\n";
...
# 	print Dumper(\%times);
	...
	die "First command occures before preceeding SNS_COMMAND ends.\nFATAL ERROR. EXIT with no output written.\n";
}
if($mm[1] > $global_stop_sec){
	...
	foreach my $i (sort {$a <=> $b} keys %times){
		...
		die "Last command occures after trailing SNS_COMMAND starts.\nFATAL ERROR. EXIT with no output written.\n";
}
...
for (my $k = 0; $k<scalar @keys4ch;$k++){
	my $n = $keys4ch[$k];
	if ($S{$n}{'type'} ne 'obs' and $S{$n}{'type'} ne 'just' and $S{$n}{'type'} ne 'just_virk' ){next;}
	if($S{$n}{'ts_mode'} =~ m/ch/i){
		# check before
		foreach my $j (0..$k-1){
			if($keys4ch[$k]-$keys4ch[$j] < 5*3600 and $S{$keys4ch[$j]}{'bands'}=~m/c/i){	# previous C-band observation was within 5 hours before the CH observations => need to power off TRST-6cm
				$t = $S{$n}{'start'}- 30*60;
				my @cmd;
		...
		foreach my $j ($k+1..$#keys4ch){
			if($keys4ch[$j]-$S{$keys4ch[$k]}{'stop'} < 5*3600 and $S{$keys4ch[$j]}{'bands'}=~m/c/i){	# next C-band observation is within 5 hours after the CH observation => need to power on TRST-6cm back
...
my $today_readable=sprintf("%02d.%02d.%02d",$today[0],$today[1],$today[2]);
my $now_readable=sprintf("%02d:%02d:%02d",$now[0],$now[1],$now[2]);
print O "//The file was created by Mikhail Lisakov on $today_readable $now_readable\n";
...
foreach(sort keys %S){
	if ($S{$_}{'type'} eq 'obs' ){
		...
		$times{$S{$_}{'start'}+59}="//\n// Observing source ".$S{$_}{'source'}.". Bands: ".$S{$_}{'bands'}.". ONA na ".$ts." ".print_time($S{$_}{'start'}+$S{$_}{'ts_bef'}*60)." - ".print_time($S{$_}{'stop'}+$S{$_}{'ts_aft'}*60)."\n//";
	}
		...
		$times{$S{$_}{'beginscan'}+59}="//\n// Observing source ".$S{$_}{'source'}.". Bands: ".$S{$_}{'bands'}.". ONA na ".$ts." ".print_time($S{$_}{'beginscan'}+$S{$_}{'ts_bef'}*60)." - ".print_time($S{$_}{'endscan'}+$S{$_}{'ts_aft'}*60)."\n//";
	}
	if ($S{$_}{'type'} eq 'just'){
		$times{$S{$_}{'beginscan'}-2}="//\n// JUSTIROVKA on source ".$S{$_}{'source'}.". Bands: ".$S{$_}{'bands'}."\n//";
	}
...
sub block_duration(){
	...
	foreach (@$ar_ref){
		if(substr($_,0,1)eq "#"){next;}		# added on 22 NOV. Allows commented lines
...
foreach my $i(@$ar_ref){
		...
		if(length($cmd)<=8){
			my $new=$n."\t".$delay."\t".$cmd."\t".$comment;		# 	WITH COMMENTS
			push @rep_array, $new;
			next;
		...
		if($repeat == 1){
			...
			my $new=$n."\t".$dt."\t".$cmd."\t".$comment;		# 	WITH COMMENTS
			push @rep_array, $new;
			my $new=$n."\t".(($delay-$dt)>$dt?($delay-$dt):$dt)."\t".$cmd."\t"."//";	# 	WITHout COMMENTS
			push @rep_array, $new;
		}
		else{
			my $new=$n."\t".$dt."\t".$cmd."\t".$comment;	# 	WITH COMMENTS
			...
			my $new=$n."\t".(($delay-$dt*($repeat-1))>=$dt?($delay-$dt*($repeat-1)):$dt)."\t".$cmd."\t"." //";
			push @rep_array, $new;
...
sub insert_block(){ # of commands
	...
	if($dir eq "-"){$shift=-$dt;}				# choose a direction to move block in case of overlapping with existing commands. A value of the shift is assigned here as well
	my @rep_array=&repeat_block($ar_ref,$repeat); 		# same array with repeated values
	...
	foreach my $c (sort keys %T){
		if(($$t_ref >= $c && $$t_ref <= $T{$c})  || (($$t_ref+$d_t_rep)>=$c && ($$t_ref+$d_t_rep)<=$T{$c}) || ($c>=$$t_ref && $T{$c}<=($$t_ref+$d_t_rep)) ){$conflict=1;}
	}
...
OUTER:	while($conflict){
	...
	foreach my $c (sort keys %T){
		...
		print print_time_only($c),"\t",print_time_only($$t_ref),"\t",print_time_only($$t_ref+$d_t_rep),"\t",print_time_only($T{$c}),"\n" if $debug2 ;

		if(($$t_ref >= $c && $$t_ref <= $T{$c})  || (($$t_ref+$d_t_rep)>=$c && ($$t_ref+$d_t_rep)<=$T{$c}) || ($c>=$$t_ref && $T{$c}<=($$t_ref+$d_t_rep))){
			$conflict=1;
...
print "block borders: start= ",print_time($$t_ref),"\tstop= ",print_time($lt),"\n" if $debug2 ;
...
OUTER:	while($conflict){
	...
	print "conf = $conflict, new_conf = $new_conflict\n" if $debug;
		...
		foreach my $kk (sort { $a <=> $b } grep { $_ == $_ } keys %times){ # loop over numerical fields of %times sorted in acsending order
			for(my $i=$$t_ref;$i<=($$t_ref+$d_t_rep);$i=$i+$dt){
				if($kk==$i){
					print "insert_block: conflict with\n\t\t",$times{$kk},"\n\t\tat ",print_time($kk),"( block duration ",$d_t_rep," from ",print_time_only($$t_ref)," to ",print_time_only($$t_ref+$d_t_rep)," )\n" if $debug;
					$conflict=1;
					$$t_ref+=$shift; 
					next OUTER;
				}
...
sub poweron{
	...
	if(any {$_=~m/p1/i} @uniq_to_poweron ){$flag+=2;}
	if(any {$_=~m/p2/i} @uniq_to_poweron ){$flag-=1;}
